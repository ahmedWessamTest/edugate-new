@extends("layouts.master")
@section('hero_image', asset('assets/images/home/hero/hero-1600.jpg'))
@section('hero_title', __('blog.education_abroad'))
@section('hero_description', __('blog.education_abroad'))
@push('styles')
  <link rel="stylesheet" href="{{asset('assets/style/text-editor.css')}}" />
  <link
      rel="stylesheet"
      href="https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/17.0.13/css/intlTelInput.css"
    />
    <style>
  /* Button Container */
  .buttons-container {
    display: flex;
    justify-content: center;
    align-items: center;
    gap: 16px;
    margin: 20px 0;
    /* flex-wrap: wrap; */
  }

  /* Base Button Style */
  .btn {
    display: inline-flex;
    align-items: center;
    justify-content: center;
    padding: 12px 24px;
    border-radius: 50px;
    font-weight: 600;
    text-decoration: none;
    font-size: 14px;
    transition: all 0.3s ease;
    min-width: 180px;
    text-align: center;
    border: none;
    cursor: pointer;
    box-shadow: 0 4px 6px rgba(0, 0, 0, 0.1);
  }

  .btn:hover {
    transform: translateY(-2px);
    box-shadow: 0 6px 8px rgba(0, 0, 0, 0.15);
  }

  .btn:active {
    transform: translateY(0);
  }

  /* WhatsApp Button */
  

 

  /* Call Now Button */
  .call-btn {
     background-color: white; /* Active color from theme */
    color: white;
  }

  .call-btn:hover {
    background-color: #e53e42;
    color: white;
  }
  /* whatsapp Now Button */
  .whatsapp {
    background-color: white;
  }

  /* Button Icon */
  .btn-icon {
    margin-inline-end: 8px;
    font-size: 16px;
  }

  /* Responsive Styles */
  @media (max-width: 768px) {
    .buttons-container {
      /* flex-direction: column; */
      gap: 12px;
      width: 100%;
    }
    
    .btn {
      width: 100%;
      max-width: 280px;
      padding: 10px 20px;
    }
    
    .content {
      padding: 0 10px;
    }
  }

  @media (max-width: 480px) {
    .btn {
      font-size: 13px;
      padding: 10px 16px;
    }
  }
</style>
@endpush
@section('content')
<section class="pb-[100px] pt-[100px] relative">
        <div class="container px-5 relative z-10 pb-[100px]">
          <div class="grid grid-cols-12 xl:gap-x-10 gap-y-10">
            <div class="col-span-12 xl:col-span-7">
              <figure class="w-full rounded-[35px] overflow-hidden">
                <img
                  src="{{asset('assets/images/related-blogs/3.webp')}}"
                  class="w-full h-full"
                  alt="blog"
                  loading="lazy"
                  decoding="async"
                />
              </figure>
              <div
                class="flex justify-between items-center gap-y-3 w-[300px] mt-6 py-5"
              >
                <span class="w-2.5 h-2.5 rounded-full bg-red-500"></span>
                <span class="w-2.5 h-2.5 rounded-full bg-red-500"></span>
                <span class="w-2.5 h-2.5 rounded-full bg-red-500"></span>
                <span class="w-2.5 h-2.5 rounded-full bg-red-500"></span>
                <span class="w-2.5 h-2.5 rounded-full bg-red-500"></span>
                <span class="w-2.5 h-2.5 rounded-full bg-red-500"></span>
              </div>
              <div
                  class="top-24 tableOfContents xl:hidden overflow-y-auto mb-5 bg-white/60 backdrop-blur-md border-s-4 border-main-green rounded-2xl shadow-md p-5 transition-all duration-300 hover:shadow-lg"
                >
                <h3 class="text-primary-text-color font-bold mb-4 text-lg tracking-wide border-b pb-2 border-gray-200">محتوي الصفحه</h3>
                <div class="custom-scrollbar max-h-[300px] ps-3 overflow-y-auto">
                  <ul class="space-y-2 text-sm text-alternate-text-color " dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}"></ul>
                </div>
              </div>
              
              <!-- TextEditorData from the database -->
              <div id="articleContainer" class="article-content">
                <p dir="rtl">
                  هنالك العديد ممن يتساءلون عن تكاليف الدراسة في الجامعات الخاصة
                  في مصر، حيث تعد هذه الجامعات خيارًا مهمًا لمن يرغبون في الحصول
                  على تعليم جامعي متميز داخل بيئة أكاديمية متطورة، وتختلف الرسوم
                  الدراسية من جامعة لأخرى وفقًا للكلية والتخصص، ومع تزايد
                  الإقبال على هذه الجامعات، أصبح من الضروري تسليط الضوء على
                  التكاليف الدراسية لمساعدة الطلاب على اتخاذ قرارات مبنية على
                  معلومات دقيق وموثوقة، ويمكنك التعرف على كافة التفاصيل المتعلقة
                  بتكاليف الدراسة في الجامعات الخاصة في مصر مع شركة ايجيك الأكثر
                  تميزًا عالميًا في الخدمات التعليمية، للتواصل اضغط على ايقونة
                  الواتساب.
                </p>

                <p dir="rtl">
                  في هذا المقال، نقدم لك دليلًا شاملًا حول كل ما يتعلق برسوم
                  الجامعات الخاصة في مصر.
                </p>

                <h2 dir="rtl" id="id-vbj24sv">
                  مصاريف الجامعات الخاصة في مصر للوافدين
                </h2>

                <p dir="rtl">
                  تتميز تكاليف الدراسة في الجامعات الخاصة في مصر بكونها رمزية
                  للغاية خاصة عند مقارنتها بالقيمة العلمية والعملية التي توفرها
                  هذه المؤسسات الأكاديمية، ومدى جودة البيئة التعليمية والبحثية
                  التي تحرص على توفيرها.
                </p>

                <p dir="rtl">
                  فرغم أن الرسوم لا تُعد مرتفعة بشكل مبالغ فيه، فإن الجامعات
                  الخاصة تحرص على تقديم برامج تعليمية حديثة ومتطورة تتوافق مع
                  المعايير العالمية، وتتواكب مع أحدث المستجدات في مختلف
                  المجالات.
                </p>

                <p dir="rtl">
                  تتمتع أيضًا الشهادات التي تمنحها هذه المؤسسات بقدر كبير من
                  التقدير والاعتراف الواسع في أسواق العمل الإقليمية والدولية،
                  مما يجعل الدراسة في هذه الجامعات خيارًا مثاليًا لكل من يرغب في
                  الحصول على تعليم عالي الجودة بتكاليف دراسية معقولة، في السطور
                  القادمة نوضح الجامعات الخاصة المعتمدة في مصر واسعارها.
                </p>

                <h3 dir="rtl">
                  أسعار الجامعات الخاصة في مصر للطلاب الوافدين 2025
                </h3>

                <p dir="rtl">
                  فيما يلي نُسلط الضوء على تكاليف الدراسة في بعض الجامعات الخاصة
                  المصرية التي تتمتع بالاعتماد والاعتراف الواسع من قبل الكثير من
                  الهيئات والجهات المرموقة:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    تتراوح رسوم الدراسة في جامعة 6 أكتوبر ما بين 2240 إلى 7500
                    دولار أمريكي سنويًا، ويبرز ذلك أن تكاليف الدراسة في الجامعات
                    الخاصة في مصر رمزية للغاية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    للدراسة في الجامعة البريطانية في مصر بمرحلة البكالوريوس،
                    يتعين على الطلاب سداد رسوم دراسية تتراوح قيمتها ما بين 2269
                    إلى 11970 جنيه إسترليني سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    يتطلب من كل من يرغب في الدراسة في الجامعة الأمريكية بالقاهرة
                    سداد رسوم دراسية قيمتها 735 دولار أمريكي لكل ساعة معتمدة.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تكون اسعار الجامعات الخاصه في جامعة أكتوبر للعلوم الحديثة
                    والآداب ما بين 6500 و 23800 دولار أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تتراوح تكاليف الدراسة في الجامعات الخاصة في مصر بجامعة مصر
                    الدولية ما بين 4000 إلى 13200 دولار أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    للدراسة في جامعة نيو جيزة، يتعين على الطلاب دفع أسعار
                    الجامعات الخاصة في مصر التي تتراوح قيمتها ما بين 10000 إلى
                    19600 دولار أمريكي لكل عام دراسي.
                  </li>
                </ul>

                <h2 dir="rtl" id="id-udf0skn">
                  مقارنة بين أسعار الجامعات الخاصة والحكومية في مصر
                </h2>

                <p dir="rtl">
                  عند المقارنة بين تكاليف الدراسة في الجامعات الخاصة في مصر
                  والجامعات الحكومية، يتضح وجود فرق كبير في مستوى المصروفات
                  الدراسية بين النوعين، ويرجع ذلك إلى اختلاف أنظمة التمويل
                  والخدمات التعليمية المقدمة في كل منهما.
                </p>

                <div></div>
                <p dir="rtl">
                  في الجامعات الحكومية، تعتبر الرسوم الدراسية منخفضة للغاية،
                  وذلك بسبب الدعم الحكومي المباشر الذي يغطي جزءً كبيرًا من رسوم
                  العملية التعليمية، حيث تتراوح رسوم الدراسة في الجامعات
                  الحكومية ما بين 3500 إلى 8000 دولار أمريكي لكل عام دراسي.
                </p>

                <p dir="rtl">
                  أما اسعار الجامعات الخاصه، فتكون أعلى في الرسوم الدراسية،
                  ويُعزى ذلك إلى اعتمادها الكامل على التمويل الذاتي، دون الحصول
                  على دعم حكومي، وتسعى هذه الجامعات إلى توفير بيئة تعليمية
                  متطورة تشمل معامل حديثة، عددًا محدودًا من الطلاب في القاعات،
                  مناهج تعليمية متطورة، وشراكات أكاديمية مع جامعات أجنبية، وتقدم
                  بعض الجامعات الخاصة برامج دولية أو ثنائية الشهادة، مما يتطلب
                  موارد إضافية تنعكس على تكاليف الدراسة في الجامعات الخاصة في
                  مصر.
                </p>

                <p dir="rtl">
                  بالتالي، فإن الفارق في أسعار الجامعات الخاصة في مصر والجامعات
                  الحكومية لا يتعلق فقط بقيمة الرسوم نفسها، بل أيضًا بنوعية
                  الخدمات التعليمية، وطبيعة البرامج، ومستوى التجهيزات داخل كل
                  مؤسسة.
                </p>

                <h2 dir="rtl" id="id-t9330rq">
                  أرخص الجامعات الخاصة في مصر وأهم مميزاتها
                </h2>

                <p dir="rtl">
                  تعد مصر موطنًا للعديد من الجامعات الخاصة التي تتميز بكونها
                  تلتزم دائمًا بإتاحة تعليم ذو جودة عالية يُلبي احتياجات كافة
                  الطلاب، إليك أبرز و<a
                    href="https://egecmena.com/ar/%D8%A7%D8%AF%D8%B1%D8%B3-%D9%81%D9%8A-%D9%85%D8%B5%D8%B1/%D8%A7%D8%B1%D8%AE%D8%B5-%D8%A7%D9%84%D8%AC%D8%A7%D9%85%D8%B9%D8%A7%D8%AA-%D8%A7%D9%84%D8%AE%D8%A7%D8%B5%D9%87-%D9%81%D9%8A-%D9%85%D8%B5%D8%B1"
                    >أرخص الجامعات الخاصة</a
                  >:
                </p>

                <h3 dir="rtl">جامعة 6 أكتوبر</h3>

                <p dir="rtl">
                  تُعد جامعة 6 أكتوبر واحدة من الجامعات الخاصة البارزة في مصر،
                  وتمكنت من ترسيخ مكانتها من خلال تبني نظام تعليمي متطور يواكب
                  التغيرات العالمية، وتُعد خيارًا مفضلًا للطلاب الوافدين
                  الراغبين في الدراسة بمصر؛ بفضل برامجها التي تجمع بين الدراسة
                  النظرية والتطبيق العملي، مما يساعد على إعداد خريجين مؤهلين
                  لسوق العمل.
                </p>

                <p dir="rtl">
                  تعد تكاليف الدراسة في الجامعات الخاصة في مصر بجامعة 6 أكتوبر
                  منخفضة، وتوفر الجامعة بيئة تعليمية حديثة، وتضم نخبة من أعضاء
                  هيئة التدريس ذوي الكفاءة، وتتيح للطلاب فرصًا للتميز العلمي
                  والمهني من خلال تخصصات متنوعة تُقدم عبر كليات مرموقة، من
                  أبرزها:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">كلية الطب.</li>
                  <li dir="rtl" style="text-align: right">كلية طب الأسنان.</li>
                  <li dir="rtl" style="text-align: right">
                    كلية العلاج الطبيعي.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية نظم المعلومات وعلوم الحاسب.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الفنون التطبيقية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الاقتصاد والإدارة.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية السياحة والفنادق.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية وسائل الإعلام والاتصالات الجماهرية.
                  </li>
                </ul>

                <h3 dir="rtl">الجامعة الأمريكية بالقاهرة</h3>

                <p dir="rtl">
                  تُعد الجامعة الأمريكية في القاهرة واحدة من أبرز المؤسسات
                  التعليمية الخاصة في مصر، وقد عززت مكانتها باعتبارها نموذجًا
                  رائدًا في تقديم التعليم الليبرالي الذي يدمج بين التميز
                  الأكاديمي وبناء الشخصية المتكاملة، وتتميز تكاليف الدراسة في
                  الجامعات الخاصة في مصر بهذه الجامعة بكونها رمزية.
                </p>

                <p dir="rtl">
                  لا تكتفي الجامعة بتقديم محتوى علمي تقليدي، بل تركز على تطوير
                  التفكير المستقل، وتعزيز مهارات التواصل، وتحفيز الطلاب على
                  الانخراط في بيئة بحثية تحفز الابتكار وتفتح آفاقًا واسعة
                  للتفكير النقدي والإبداعي، وتقدم الجامعة برامجها من خلال مجموعة
                  من الكليات المتخصصة التي تشمل:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    كلية العلوم والهندسة.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية العلوم الإنسانية والاجتماعية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الشؤون العالمية والسياسات العامة.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية أنسي ساويرس لإدارة الأعمال.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية التعليم المستمر.
                  </li>
                </ul>

                <h3 dir="rtl">الجامعة البريطانية في مصر</h3>

                <div></div>
                <p dir="rtl">
                  تسعى الجامعة البريطانية في مصر إلى تقديم تجربة تعليمية متكاملة
                  تقوم على الأسس الأكاديمية البريطانية التي تشتهر بتشجيع التفكير
                  التحليلي وتعزيز القدرات الإبداعية لدى الطلاب، وتعتبر تكاليف
                  الدراسة في الجامعات الخاصة في مصر بالجامعة البريطانية معقولة.
                </p>

                <p dir="rtl">
                  لا تقتصر رسالتها على مجرد تقديم المحتوى العلمي، بل تمتد لتشمل
                  إعداد طلاب يتمتعون بمهارات عالية في النقد واتخاذ القرار،
                  وقادرين على التعامل مع التحديات المعقدة بطرق غير تقليدية.
                </p>

                <p dir="rtl">
                  تحرص الجامعة على توفير مناخ أكاديمي متطور يدعم الاستقلالية في
                  التعلم ويُعزز من قدرة الطلاب على التفاعل مع الواقع المهني
                  والعلمي بمرونة وابتكار، وتضم الجامعة مجموعة من الكليات الحديثة
                  التي تتضمن:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">كلية طب الأسنان.</li>
                  <li dir="rtl" style="text-align: right">
                    كلية العلاج الطبيعي.
                  </li>
                  <li dir="rtl" style="text-align: right">كلية التمريض.</li>
                  <li dir="rtl" style="text-align: right">كلية الهندسة.</li>
                  <li dir="rtl" style="text-align: right">
                    كلية هندسة الطاقة والبيئة.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية إدارة الأعمال والاقتصاد والعلوم السياسية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الاتصالات ووسائل الإعلام.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الفنون والعلوم الإنسانية.
                  </li>
                </ul>

                <h3 dir="rtl">جامعة المستقبل</h3>

                <p dir="rtl">
                  تحظى جامعة المستقبل بمكانة أكاديمية مرموقة على الصعيدين المحلي
                  والدولي، حيث تتميز بكونها تتيح بيئة تعليمية متقدمة وبرامج
                  أكاديمية متطورة تتلائم مع أرقى النظم والمعايير العالمية، وتتسم
                  تكاليف الدراسة في الجامعات الخاصة في مصر وخصوصًا جامعة
                  المستقبل بأنها معتدلة.
                </p>

                <p dir="rtl">
                  تهدف جامعة المستقبل إلى تزويد الطلاب بثروات معلوماتية قيمة
                  وخبرات واسعة تُمكّنهم من العمل بجدارة في مختلف القطاعات في
                  كافة أسواق العمل الإقليمية والدولية، وتتيح الجامعة مجموعة
                  واسعة من البرامج التعليمية من خلال كلياتها التي تتمثل فيما
                  يلي:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    كلية طب الفم والأسنان.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية العلوم الصيدلانية والصناعات الدوائية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الهندسة وتقنية المعلومات.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الاقتصاد والعلوم السياسية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية التجارة وإدارة الأعمال.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الحاسبات وتقنية المعلومات.
                  </li>
                </ul>

                <h3 dir="rtl">جامعة مصر للعلوم والتكنولوجيا</h3>

                <p dir="rtl">
                  تتميز جامعة مصر للعلوم والتكنولوجيا بكونها من أوائل الجامعات
                  الخاصة في مصر التي تقدم تجربة تعليمية متقدمة تدمج بين الجودة
                  الأكاديمية والمرونة في التعلم، وتوفر بيئة تعليمية تشجع البحث
                  العلمي والتفكير النقدي، وتعد تكاليف الدراسة في الجامعات الخاصة
                  في مصر وخاصة في هذه الجامعة معقولة.
                </p>

                <p dir="rtl">
                  تركز الجامعة على إعداد خريجين متميزين أكاديميًا ومهنيًا قادرين
                  على ربط المعرفة بالتطبيق العملي وذلك من خلال كلياتها المرموقة،
                  إليك أبرز هذه الكليات:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">كلية الطب</li>
                  <li dir="rtl" style="text-align: right">
                    كلية طب وجراحة الفم والأسنان
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية العلوم الصيدلية والتصنيع الغذائي
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية العلاج الطبيعي
                  </li>
                  <li dir="rtl" style="text-align: right">كلية التمريض</li>
                  <li dir="rtl" style="text-align: right">
                    كلية العلوم الهندسية والتكنولوجيا
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الإدارة والاقتصاد ونظم المعلومات
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية الإعلام وتكنولوجيا الاتصال
                  </li>
                  <li dir="rtl" style="text-align: right">
                    كلية اللغات والترجمة
                  </li>
                </ul>

                <h2 dir="rtl" id="id-ikffx30">
                  أسعار دخول الجامعات الخاصة حسب التخصص (طب – هندسة – إدارة
                  أعمال...)
                </h2>

                <p dir="rtl">
                  تتميز الجامعات الخاصة المصرية بأنها تقدم مجموعة متعددة من
                  التخصصات، مما يتيح الفرصة للطلاب للحصول على التخصص الذي يتوافق
                  مع تطلعاتهم وأهدافهم الأكاديمية والمهنية، فيما يلي نُسلط الضوء
                  على تكاليف الدراسة في الجامعات الخاصة في مصر بالتخصصات
                  المختلفة في مرحلة البكالوريوس:
                </p>

                <h3 dir="rtl">رسوم تخصصات جامعة 6 أكتوبر</h3>

                <div></div>
                <p dir="rtl">
                  إليك تكاليف الدراسة في الجامعات الخاصة في مصر بجامعة 6 أكتوبر
                  في مختلف التخصصات التي تتيحها الجامعة:
                </p>

                <p style="text-align: right">&nbsp;</p>

                <div dir="rtl">
                  <table cellspacing="0">
                    <tbody>
                      <tr>
                        <td colspan="2">
                          <p dir="rtl">التخصص</p>
                        </td>
                        <td>
                          <p dir="rtl">التكلفة بالدولار الأمريكي</p>
                        </td>
                        <td colspan="2">
                          <p dir="rtl">التخصص</p>
                        </td>
                        <td>
                          <p dir="rtl">التكلفة بالدولار أمريكي</p>
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2">
                          <p dir="rtl">الطب والجراحة</p>
                        </td>
                        <td>
                          <p dir="rtl">7500$</p>
                        </td>
                        <td colspan="2">
                          <p dir="rtl">الهندسة</p>
                        </td>
                        <td>
                          <p dir="rtl">4200$</p>
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2">
                          <p dir="rtl">طب الأسنان</p>
                        </td>
                        <td>
                          <p dir="rtl">6500$</p>
                        </td>
                        <td colspan="2">
                          <p dir="rtl">نظم المعلومات وعلوم الحاسب</p>
                        </td>
                        <td>
                          <p dir="rtl">3500$</p>
                        </td>
                      </tr>
                      <tr>
                        <td rowspan="2">
                          <p>&nbsp;</p>

                          <p dir="rtl">الصيدلة</p>
                        </td>
                        <td>
                          <p>Pharm D</p>
                        </td>
                        <td>
                          <p dir="rtl">5600$</p>
                        </td>
                        <td rowspan="2">
                          <p>&nbsp;</p>

                          <p dir="rtl">الاقتصاد</p>
                        </td>
                        <td>
                          <p dir="rtl">قسم عربي</p>
                        </td>
                        <td>
                          <p dir="rtl">2800$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p>Pharm D Clinical</p>
                        </td>
                        <td>
                          <p dir="rtl">6000$</p>
                        </td>
                        <td>
                          <p dir="rtl">قسم إنجليزي</p>
                        </td>
                        <td>
                          <p dir="rtl">3700$</p>
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2">
                          <p dir="rtl">العلاج الطبيعي</p>
                        </td>
                        <td>
                          <p dir="rtl">4200$</p>
                        </td>
                        <td colspan="2">
                          <p dir="rtl">اللغات والترجمة</p>
                        </td>
                        <td>
                          <p dir="rtl">2940$</p>
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2">
                          <p dir="rtl">تكنولوجيا العلوم الصحية التطبيقية</p>
                        </td>
                        <td>
                          <p dir="rtl">3500$</p>
                        </td>
                        <td colspan="2">
                          <p dir="rtl">الإعلام ووسائل الاتصال</p>
                        </td>
                        <td>
                          <p dir="rtl">2800$</p>
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2">
                          <p dir="rtl">التمريض</p>
                        </td>
                        <td>
                          <p dir="rtl">3000$</p>
                        </td>
                        <td colspan="2">
                          <p dir="rtl">السياحة والفنادق</p>
                        </td>
                        <td>
                          <p dir="rtl">2500:2240$</p>
                        </td>
                      </tr>
                      <tr>
                        <td colspan="2">
                          <p dir="rtl">التربية</p>
                        </td>
                        <td>
                          <p dir="rtl">2240$</p>
                        </td>
                        <td colspan="2">
                          <p dir="rtl">الفنون التطبيقية</p>
                        </td>
                        <td>
                          <p dir="rtl">3150$</p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>

                <h3 dir="rtl">رسوم تخصصات الجامعة البريطانية في مصر</h3>

                <p dir="rtl">
                  فيما يلي نوضح تكاليف الدراسة في الجامعات الخاصة في مصر
                  بالجامعة البريطانية في مختلف التخصصات التي تتيحها الجامعة
                  البريطانية بمصر:
                </p>

                <p style="text-align: right">&nbsp;</p>

                <div dir="rtl">
                  <table cellspacing="0">
                    <tbody>
                      <tr>
                        <td>
                          <p dir="rtl">التخصصات</p>
                        </td>
                        <td>
                          <p dir="rtl">التكلفة بالجنيه الاسترليني</p>
                        </td>
                        <td>
                          <p dir="rtl">التخصصات</p>
                        </td>
                        <td>
                          <p dir="rtl">التكلفة بالجنيه الاسترليني</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">طب الأسنان</p>
                        </td>
                        <td>
                          <p dir="rtl">11970:10000</p>
                        </td>
                        <td>
                          <p dir="rtl">الفنون والتصميم</p>
                        </td>
                        <td>
                          <p dir="rtl">8700:6388</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الصيدلة السريرية</p>

                          <p>Pharm D - Clinical</p>
                        </td>
                        <td>
                          <p dir="rtl">7950:5338</p>
                        </td>
                        <td>
                          <p dir="rtl">إدارة الأعمال</p>
                        </td>
                        <td>
                          <p dir="rtl">9600:6928</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الصيدلة Pharm-D</p>
                        </td>
                        <td>
                          <p dir="rtl">7600:5128</p>
                        </td>
                        <td>
                          <p dir="rtl">العلوم السياسية</p>
                        </td>
                        <td>
                          <p dir="rtl">6900:5308</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">العلاج الطبيعي</p>
                        </td>
                        <td>
                          <p dir="rtl">8350:6178</p>
                        </td>
                        <td>
                          <p dir="rtl">الاقتصاد</p>
                        </td>
                        <td>
                          <p dir="rtl">6900:5308</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">التمريض</p>
                        </td>
                        <td>
                          <p dir="rtl">2900:2268</p>
                        </td>
                        <td>
                          <p dir="rtl">الإعلام والاتصال</p>
                        </td>
                        <td>
                          <p dir="rtl">7070:5410</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الهندسة</p>
                        </td>
                        <td>
                          <p dir="rtl">10500:7468</p>
                        </td>
                        <td>
                          <p dir="rtl">علم النفس</p>
                        </td>
                        <td>
                          <p dir="rtl">7950:5938</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الطاقة والهندسة البيئية</p>
                        </td>
                        <td>
                          <p dir="rtl">8300:6148</p>
                        </td>
                        <td>
                          <p dir="rtl">القانون</p>
                        </td>
                        <td>
                          <p dir="rtl">6900:5308</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">علوم الحاسب الآلي</p>
                        </td>
                        <td>
                          <p dir="rtl">9400:6808</p>
                        </td>
                        <td rowspan="2">
                          <p dir="rtl">اللغة والثقافة الصينية</p>
                        </td>
                        <td rowspan="2">
                          <p dir="rtl">3260:2444</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">اللغة الإنجليزية وآدابها</p>
                        </td>
                        <td>
                          <p dir="rtl">4700:3710</p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>

                <p style="text-align: right">&nbsp;</p>

                <h3 dir="rtl">
                  رسوم تخصصات جامعة أكتوبر للعلوم الحديثة والآداب
                </h3>

                <p dir="rtl">
                  إليك قائمة بتكاليف الدراسة في الجامعات الخاصة في مصر بجامعة
                  أكتوبر للعلوم الحديثة والآداب:
                </p>

                <p style="text-align: right">&nbsp;</p>

                <div dir="rtl">
                  <table cellspacing="0">
                    <tbody>
                      <tr>
                        <td>
                          <p dir="rtl">التخصص</p>
                        </td>
                        <td>
                          <p dir="rtl">الرسوم السنوية بالدولار</p>
                        </td>
                        <td>
                          <p dir="rtl">التخصص</p>
                        </td>
                        <td>
                          <p dir="rtl">الرسوم السنوية بالدولار</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">طب الأسنان</p>
                        </td>
                        <td>
                          <p dir="rtl">23800:6360$</p>
                        </td>
                        <td>
                          <p dir="rtl">الهندسة</p>
                        </td>
                        <td>
                          <p dir="rtl">15800:4600$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">العلاج الطبيعي</p>
                        </td>
                        <td>
                          <p dir="rtl">18800:5460$</p>
                        </td>
                        <td>
                          <p dir="rtl">علوم الحاسب</p>
                        </td>
                        <td>
                          <p dir="rtl">15800:3860$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الصيدلة</p>
                        </td>
                        <td>
                          <p dir="rtl">18800:5460$</p>
                        </td>
                        <td>
                          <p dir="rtl">الفنون والتصميم</p>
                        </td>
                        <td>
                          <p dir="rtl">15800:4600$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">التكنولوجيا الحيوية</p>
                        </td>
                        <td>
                          <p dir="rtl">17800:5200$</p>
                        </td>
                        <td>
                          <p dir="rtl">الإعلام</p>
                        </td>
                        <td>
                          <p dir="rtl">13200:3860$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">التغذية وتكنولوجيا الأغذية</p>
                        </td>
                        <td>
                          <p dir="rtl">17800:5200$</p>
                        </td>
                        <td rowspan="2">
                          <p dir="rtl">اللغات</p>
                        </td>
                        <td rowspan="2">
                          <p dir="rtl">11200:3250$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">العلوم الإدارية</p>
                        </td>
                        <td>
                          <p dir="rtl">13200:3860$</p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>

                <p style="text-align: right">&nbsp;</p>

                <h2 dir="rtl" id="id-6vb7vqm">
                  مصاريف الجامعات الخاصة طب بشري
                </h2>

                <div></div>
                <p dir="rtl">
                  يوجد العديد من الجامعات الخاصة التي تتيح دراسة تخصص الطب
                  البشري، حيث تقدم تعليم طبي عالي الجودة يتناسب مع أحدث التطورات
                  في المجال الطبي، ويمد الدارسين بكافة العلوم الطبية والمهارات
                  التي تُمكّنهم من العمل بجدارة في شتى القطاعات الطبية، وفيما
                  يلي نوضح تكاليف الدراسة في الجامعات الخاصة في مصر بتخصص الطب
                  البشري:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    تبلغ تكلفة دراسة الطب البشري في جامعة 6 أكتوبر 7500 دولار
                    أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تكون رسوم دراسة الطب البيطري في جامعة النهضة 7000 دولار
                    أمريكي لكل عام دراسي.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تصل رسوم دراسة الطب البشري في جامعة نيو جيزة إلى 19600 دولار
                    أمريكي.
                  </li>
                </ul>

                <p dir="rtl">
                  يمكنك معرفة كافة التفاصيل التي تتعلق بمصاريف الجامعات الخاصة
                  في مصر للوافدين لدراسة الطب البشري من خلال التواصل مع فريق
                  شركة ايجيك الذي يمتلك خبرة واسعة في مجال الخدمات التعليمية،
                  للتواصل اضغط على ايقونة الواتساب.
                </p>

                <h2 dir="rtl" id="id-05d8naq">مصاريف جامعة مصر الدولية</h2>

                <p dir="rtl">
                  تعتبر جامعة مصر الدولية من أبرز الجامعات الرائدة في مصر التي
                  تتيح مجموعة متنوعة من البرامج الأكاديمية في مختلف التخصصات
                  الأكثر طلبًا في أسواق العمل على المستويين المحلي والدولي،
                  وتتسم مصاريف الدراسة في هذه الجامعة بأنها منخفضة نسبيًا
                  وملائمة إلى حد كبير مع الطلاب، إليك تكاليف الدراسة في الجامعات
                  الخاصة في مصر بجامعة مصر الدولية:
                </p>

                <p style="text-align: right">&nbsp;</p>

                <div dir="rtl">
                  <table cellspacing="0">
                    <tbody>
                      <tr>
                        <td>
                          <p dir="rtl">الكلية</p>
                        </td>
                        <td>
                          <p dir="rtl">معدل القبول</p>
                        </td>
                        <td>
                          <p dir="rtl">
                            الرسوم الدراسية بالدولار الأمريكي لكل فصل دراسي
                          </p>
                        </td>
                      </tr>
                      <tr>
                        <td rowspan="4">
                          <p>&nbsp;</p>

                          <p dir="rtl">الصيدلة</p>
                        </td>
                        <td>
                          <p dir="rtl">80%</p>
                        </td>
                        <td>
                          <p dir="rtl">2750$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">75%</p>
                        </td>
                        <td>
                          <p dir="rtl">3000$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">70%</p>
                        </td>
                        <td>
                          <p dir="rtl">3250$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">أقل من 70%</p>
                        </td>
                        <td>
                          <p dir="rtl">4850$</p>
                        </td>
                      </tr>
                      <tr>
                        <td rowspan="4">
                          <p>&nbsp;</p>

                          <p dir="rtl">طب الفم والأسنان</p>
                        </td>
                        <td>
                          <p dir="rtl">85%</p>
                        </td>
                        <td>
                          <p dir="rtl">5775$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">80%</p>
                        </td>
                        <td>
                          <p dir="rtl">6050$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">75%</p>
                        </td>
                        <td>
                          <p dir="rtl">6325$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">أقل من 75%</p>
                        </td>
                        <td>
                          <p dir="rtl">6600$</p>
                        </td>
                      </tr>
                      <tr>
                        <td rowspan="4">
                          <p>&nbsp;</p>

                          <p dir="rtl">العلوم الهندسية والفنون</p>
                        </td>
                        <td>
                          <p dir="rtl">80%</p>
                        </td>
                        <td>
                          <p dir="rtl">2750$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">75%</p>
                        </td>
                        <td>
                          <p dir="rtl">3000$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">70%</p>
                        </td>
                        <td>
                          <p dir="rtl">3250$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">أقل من 70%</p>
                        </td>
                        <td>
                          <p dir="rtl">4850$</p>
                        </td>
                      </tr>
                      <tr>
                        <td rowspan="4">
                          <p>&nbsp;</p>

                          <p dir="rtl">علوم الحاسوب</p>
                        </td>
                        <td>
                          <p dir="rtl">80%</p>
                        </td>
                        <td>
                          <p dir="rtl">2750$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">75%</p>
                        </td>
                        <td>
                          <p dir="rtl">3000$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">70%</p>
                        </td>
                        <td>
                          <p dir="rtl">3250$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">أقل من 70%</p>
                        </td>
                        <td>
                          <p dir="rtl">4000$</p>
                        </td>
                      </tr>
                      <tr>
                        <td rowspan="4">
                          <p>&nbsp;</p>

                          <p dir="rtl">إدارة الأعمال والتجارة الدولية</p>
                        </td>
                        <td>
                          <p dir="rtl">80%</p>
                        </td>
                        <td>
                          <p dir="rtl">2500$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">75%</p>
                        </td>
                        <td>
                          <p dir="rtl">2750$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">70%</p>
                        </td>
                        <td>
                          <p dir="rtl">3000$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">أقل من 70%</p>
                        </td>
                        <td>
                          <p dir="rtl">4250$</p>
                        </td>
                      </tr>
                      <tr>
                        <td rowspan="4">
                          <p>&nbsp;</p>

                          <p dir="rtl">الاتصال الجماهيري</p>
                        </td>
                        <td>
                          <p dir="rtl">80%</p>
                        </td>
                        <td>
                          <p dir="rtl">2500$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">75%</p>
                        </td>
                        <td>
                          <p dir="rtl">2750$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">70%</p>
                        </td>
                        <td>
                          <p dir="rtl">3000$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">أقل من 70%</p>
                        </td>
                        <td>
                          <p dir="rtl">4250$</p>
                        </td>
                      </tr>
                      <tr>
                        <td rowspan="4">
                          <p>&nbsp;</p>

                          <p dir="rtl">اللغويات التطبيقية (الألسن)</p>
                        </td>
                        <td>
                          <p dir="rtl">80%</p>
                        </td>
                        <td>
                          <p dir="rtl">2000$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">75%</p>
                        </td>
                        <td>
                          <p dir="rtl">2125$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">70%</p>
                        </td>
                        <td>
                          <p dir="rtl">2250$</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">أقل من 70%</p>
                        </td>
                        <td>
                          <p dir="rtl">3750$</p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>

                <p style="text-align: right">&nbsp;</p>

                <h2 dir="rtl" id="id-wcbohf4">
                  المنح الدراسية المتاحة في الجامعات الخاصة للطلاب الوافدين في
                  مصر
                </h2>

                <p dir="rtl">
                  تتيح الجامعات الخاصة العديد من المنح الدراسية؛ رغبةً منها في
                  تقليل تكاليف الدراسة في الجامعات الخاصة في مصر من أجل إزالة
                  الأعباء المالية التي قد تحول بين الدارسين وبين تحقيق أحلامهم،
                  وفيما يلي نُسلط الضوء على أبرز المنح المتاحة عند الدراسة في
                  الجامعات الخاصة:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    برنامج منح التميز في الجامعة الأمريكية بالقاهرة: يعد من أبرز
                    الفرص التي تقدمها الجامعة لدعم الطلاب أكاديميًا، حيث يتيح
                    للطلاب الحصول على إعفاء جزئي أو كلي من الرسوم الدراسية،
                    بخصومات تبدأ من 20% وتصل حتى 100%، وذلك تبعًا لشروط ومعايير
                    معينة تضعها الجامعة.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    منح جامعة مصر الدولية: تتيح الجامعة العديد من المنح التي
                    تسهم في تخفيض الرسوم الدراسية على الطلاب.
                  </li>
                </ul>

                <p dir="rtl">
                  يمكنك معرفة كافة التفاصيل حول منح الجامعات الخاصة التي تسهم في
                  تقليل تكاليف الدراسة في الجامعات الخاصة في مصر من خلال التواصل
                  مع شركة ايجيك صاحبة الخبرة الواسعة في مجال الخدمات التعليمية،
                  فقط اضغط على ايقونة الواتساب واحصل على استشارات مجانية.
                </p>

                <h2 dir="rtl" id="id-wgg558t">
                  خطوات التقديم في الجامعات الخاصة للطلاب العرب
                </h2>

                <p dir="rtl">
                  بعد معرفة تكاليف الدراسة في الجامعات الخاصة في مصر، يمكنك
                  التقديم في هذه الجامعات بكل سلاسة عبر شركة ايجيك الأكثر تميزًا
                  عالميًا في مجال الخدمات التعليمية، حيث تقوم بإتمام كافة
                  المعاملات والإجراءات اللازمة للتقديم بالنيابة عن الدارسين، مما
                  يوفر عليهم الجهد والوقت.
                </p>

                <div></div>
                <p dir="rtl">
                  تواصل مع ايجيك من خلال الضغط على ايقونة الواتساب واتبع الخطوات
                  التالية لضمان عملية تقديم ناجحة وخالية من المتاعب:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    إعداد صور المستندات اللازمة للتقديم هي الجامعات الخاصة والتي
                    تتضمن:
                    <ul>
                      <li dir="rtl">
                        صورة شهادة الثانوية العامة أو ما يعادلها وكشف الدرجات.
                      </li>
                      <li dir="rtl">
                        صورة شهادة البكالوريوس وبيان الدرجات (للتقديم على
                        الماجستير)
                      </li>
                      <li dir="rtl">
                        صورة شهادة الماجستير وبيان الدرجات (للتقديم على
                        الدكتوراه)
                      </li>
                      <li dir="rtl">صورة شهادة الميلاد</li>
                      <li dir="rtl">صورة جواز سفر ساري</li>
                    </ul>
                  </li>
                  <li dir="rtl" style="text-align: right">
                    بعد ظهور الترشيح المبدئي، يلزم تقديم النسخ الأصلية من
                    المستندات المطلوبة، وهي:
                    <ul>
                      <li dir="rtl">
                        أصل شهادة الثانوية أو ما يعادلها وبيان الدرجات
                      </li>
                      <li dir="rtl">
                        أصل شهادة البكالوريوس وبيان الدرجات (للمتقدمين على
                        الماجستير)
                      </li>
                      <li dir="rtl">
                        أصل شهادة الماجستير وبيان الدرجات (للمتقدمين على
                        الدكتوراه)
                      </li>
                      <li dir="rtl">6 صور شخصية</li>
                      <li dir="rtl">
                        أصل الوكال لإتمام الإجراءات الضرورية للتقديم في الجامعات
                        المصرية
                      </li>
                    </ul>
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تُرسل كافة المستندات عبر أحد المكاتب التالي ذكرها:
                    <ul>
                      <li dir="rtl">بريد دولة الوافد</li>
                      <li dir="rtl">
                        <a
                          href="https://en.wikipedia.org/wiki/DHL"
                          target="_blank"
                          >DHL</a
                        >
                      </li>
                      <li dir="rtl">Aramex</li>
                      <li dir="rtl">FedEX</li>
                      <li dir="rtl">SMSA</li>
                    </ul>
                  </li>
                </ul>

                <h2 dir="rtl" id="id-sdxk6cp">
                  ترتيب الجامعات الخاصة في مصر عالميًا
                </h2>

                <p dir="rtl">
                  بعد التعرف على تكاليف الدراسة في الجامعات الخاصة في مصر، سنطلع
                  على تصنيف هذه الجامعات، حيث تميزت
                  <a
                    href="https://egecmena.com/ar/%D8%A7%D8%AF%D8%B1%D8%B3-%D9%81%D9%8A-%D9%85%D8%B5%D8%B1/%D8%A7%D8%B3%D9%85%D8%A7%D8%A1-%D8%A7%D9%84%D8%AC%D8%A7%D9%85%D8%B9%D8%A7%D8%AA-%D8%A7%D9%84%D8%AE%D8%A7%D8%B5%D8%A9-%D8%A7%D9%84%D9%85%D8%B9%D8%AA%D9%85%D8%AF%D8%A9-%D9%81%D9%8A-%D9%85%D8%B5%D8%B1"
                    >الجامعات الخاصة المصرية</a
                  >
                  بحصولها على مجموعة متعددة من المراكز المتقدمة في مختلف
                  التصنيفات العالمية، مثل: تصنيف QS والتايمز، مما يظهر ذلك مدى
                  التزام هذه المؤسسات التعليمية بتطبيق معايير الجودة، ومدى حرصها
                  على توفير بيئة أكاديمية تتلاءم مع المعايير العالمية، فيما يلي
                  نذكر ترتيب بعض الجامعات الخاصة المصرية في تصنيف QS للجامعات
                  العالمية:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    حصلت الجامعة الأمريكية بالقاهرة في تصنيف QS على المرتبة 381
                    على مستوى الجامعات العالمية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    نالت جامعة المستقبل في مصر المرتبة 901-950 وفقًا لتصنيف QS
                    للجامعات العالمية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جاءت الجامعة البريطانية في مصر في المرتبة 1201-1400 طبقًا
                    لتصنيف QS للجامعات العالمية.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    حظت جامعة الدلتا للعلوم والتكنولوجيا بالمركز 1201-1400.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    حصلت الجامعة الألمانية بالقاهرة على المركز 1201-1400 حسب
                    تصنيف QS على مستوى الجامعات العالمية.
                  </li>
                </ul>

                <h2 dir="rtl" id="id-zsj0m3y">
                  الجامعات الخاصة المعتمدة من وزارة التعليم العالي
                </h2>

                <p dir="rtl">
                  تضم مصر العديد من الجامعات الخاصة المعتمدة من وزارة التعليم
                  العالي، والتي تمثل خيارًا تعليميًا متميزًا للطلاب الراغبين في
                  دراسة تخصصات متنوعة داخل بيئة تعليمية حديثة، وتتمتع هذه
                  الجامعات باعتراف رسمي يجعل شهاداتها معادلة لتلك التي تصدرها
                  الجامعات الحكومية، ما يضمن قبولها في سوق العمل المحلي والدولي.
                </p>

                <p dir="rtl">
                  من أبرز مميزات الجامعات الخاصة في مصر أنها توفر برامج دراسية
                  مرنة ومتطورة، تعتمد على أحدث المناهج وأساليب التدريس، إلى جانب
                  اهتمامها بالتدريب العملي وربط الطالب بالتطبيقات الواقعية في
                  مجاله.
                </p>

                <p dir="rtl">
                  تتعاون العديد من هذه الجامعات مع مؤسسات أكاديمية دولية، ما
                  يتيح للطلاب فرصًا للحصول على تعليم مزدوج، وتتسم أيضًا تكاليف
                  الدراسة في الجامعات الخاصة في مصر بكونها رمزية ومناسبة مع
                  الطلاب.
                </p>

                <p dir="rtl">
                  وتُعرف الجامعات الخاصة أيضًا ببيئتها الجامعية المتطورة، حيث
                  تهتم بالبنية التحتية الحديثة، والمختبرات المجهزة، والأنشطة
                  الطلابية المتنوعة، مما يساهم في تطوير المهارات الشخصية
                  والأكاديمية للطلاب، إليك أبرز الجامعات الخاصة المعتمدة من قبل
                  وزارة التعليم العالي:
                </p>

                <p style="text-align: right">&nbsp;</p>

                <div dir="rtl">
                  <table cellspacing="0">
                    <tbody>
                      <tr>
                        <td>
                          <p dir="rtl">جامعة 6 أكتوبر</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة الأهرام الكندية</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة الدلتا للعلوم والتكنولوجيا</p>
                        </td>
                        <td>
                          <p dir="rtl">الجامعة الألمانية في مصر</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">جامعة مصر للعلوم والتكنولوجيا</p>
                        </td>
                        <td>
                          <p dir="rtl">الجامعة الأمريكية بالقاهرة</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة بدر بالقاهرة</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة سيناء</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">جامعة مصر الدولية</p>
                        </td>
                        <td>
                          <p dir="rtl">الجامعة البريطانية في مصر</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة دراية</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة فاروس</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">جامعة أكتوبر للعلوم الحديثة والآداب</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة المستقبل</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة النهضة</p>
                        </td>
                        <td>
                          <p dir="rtl">جامعة الجيزة الجديدة</p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>

                <h2 dir="rtl" id="id-5iovkch">
                  أسعار كليات الهندسة وإدارة الأعمال في الجامعات الخاصة
                </h2>

                <p dir="rtl">
                  تقدم العديد من الجامعات الخاصة في مصر برامج أكاديمية في
                  الهندسة وإدارة الأعمال، وذلك بهدف تزويد الطلاب بكافة المعارف
                  الهندسية والإدارية التي تُمكّنهم من العمل بكفاءة في مختلف
                  القطاعات الهندسية والإدارية على المستويين المحلي والدولي.
                </p>

                <div></div>
                <p dir="rtl">
                  تتميز تكاليف الدراسة في الجامعات الخاصة في مصر بتخصصات الهندسة
                  وإدارة الأعمال بكونها معتدلة عند مقارنتها بجودة البيئة
                  التعليمية التي توفرها هذه الجامعات، وفيما يلي نوضح اسعار
                  الجامعات الخاصه بكليات الهندسة وإدارة الأعمال:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    تتراوح رسوم دراسة الهندسة في الجامعة البريطانية في مصر ما
                    بين 6148 إلى 10500 جنيه استرليني سنويًا، بينما تكون تكلفة
                    إدارة الأعمال ما بين 6928 إلى 9600 جنيه استرليني سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تكون تكاليف الدراسة في الجامعات الخاصة في مصر لدراسة الهندسة
                    في جامعة أكتوبر للعلوم الحديثة والآداب ما بين 4600 إلى 15800
                    دولار أمريكي سنويًا، بينما تتراوح دراسة رسوم دراسة العلوم
                    الإدارية ما بين 3860 إلى 13200 دولار أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تصل مصاريف الجامعات الخاصة في مصر للوافدين لدراسة الهندسة في
                    جامعة 6 أكتوبر إلى 4200 دولار أمريكي لكل عام دراسي.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تبلغ رسوم الساعة المعتمدة لدراسة الهندسة وإدارة الأعمال في
                    الجامعة الأمريكية بالقاهرة 735 دولار أمريكي.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    تكون رسوم دراسة الهندسة في جامعة نيو جيزة 13200 دولار أمريكي
                    سنويًا، بينما تصل رسوم دراسة إدارة الأعمال بنفس الجامعة إلى
                    10000 دولار أمريكي لكل عام دراسي.
                  </li>
                </ul>

                <h2 dir="rtl" id="id-12ek5nq">
                  قائمة مصاريف الجامعات الخاصة 2025/2026
                </h2>

                <p dir="rtl">
                  تختلف تكاليف الدراسة في الجامعات الخاصة في مصر وفقًا لعدة
                  عوامل، منها: الجامعة التي يطمح الطالب إلى الالتحاق بالدراسة
                  بها، ونوع التخصص، والدرجة العلمية التي يسعى إلى الحصول عليها،
                  فيما يلي قائمة بالجامعات الخاصة المعتمدة في مصر واسعارها:
                </p>

                <div dir="rtl">
                  <table cellspacing="0">
                    <tbody>
                      <tr>
                        <td>
                          <p dir="rtl">الجامعة</p>
                        </td>
                        <td>
                          <p dir="rtl">المصاريف الدراسية</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">جامعة 6 أكتوبر</p>
                        </td>
                        <td>
                          <p dir="rtl">7500:2240 دولار أمريكي سنويًا</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الجامعة البريطانية في مصر</p>
                        </td>
                        <td>
                          <p dir="rtl">11970:2269 جنيه إسترليني سنويًا</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الجامعة الأمريكية بالقاهرة</p>
                        </td>
                        <td>
                          <p dir="rtl">735 دولار أمريكي لكل ساعة معتمدة</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">جامعة أكتوبر للعلوم الحديثة والآداب</p>
                        </td>
                        <td>
                          <p dir="rtl">23800:6500 دولار أمريكي سنويًا</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">جامعة مصر الدولية</p>
                        </td>
                        <td>
                          <p dir="rtl">13200:4000 دولار أمريكي سنويًا</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">جامعة نيو جيزة</p>
                        </td>
                        <td>
                          <p dir="rtl">19600:10000 دولار أمريكي سنويًا</p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>

                <h2 dir="rtl" id="id-80tvtj0">تنسيق الجامعات الخاصة 2025</h2>

                <p dir="rtl">
                  تُعرف الجامعات المصرية بكونها تتيح العديد من التسهيلات للطلاب،
                  من ضمن تلك التسهيلات، أن تكاليف الدراسة في الجامعات الخاصة في
                  مصر رمزية للغاية وفي متناول معظم الطلاب، بالإضافة إلى ذلك
                  تعتبر
                  <a
                    href="https://egecmena.com/ar/%D8%A7%D8%AF%D8%B1%D8%B3-%D9%81%D9%8A-%D9%85%D8%B5%D8%B1/%D8%A3%D9%81%D8%B6%D9%84-%D8%A7%D9%84%D8%AC%D8%A7%D9%85%D8%B9%D8%A7%D8%AA-%D8%A7%D9%84%D8%AE%D8%A7%D8%B5%D8%A9-%D9%81%D9%8A-%D9%85%D8%B5%D8%B1"
                    >معدلات القبول</a
                  >
                  في الجامعات الخاصة منخفضة نسبيًا، مما يمنح الطلاب الفرصة
                  الأكبر للالتحاق بالتخصصات الراغبين فيها وتحقيق أهدافهم
                  الأكاديمية والمهنية، إليك تنسيق الجامعات المصرية:
                </p>

                <p style="text-align: right">&nbsp;</p>

                <div dir="rtl">
                  <table cellspacing="0">
                    <tbody>
                      <tr>
                        <td>
                          <p dir="rtl">الكلية</p>
                        </td>
                        <td>
                          <p dir="rtl">معدل القبول</p>
                        </td>
                        <td>
                          <p dir="rtl">الكلية</p>
                        </td>
                        <td>
                          <p dir="rtl">معدل القبول</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الطب البشري</p>
                        </td>
                        <td>
                          <p dir="rtl">70%:75%</p>
                        </td>
                        <td>
                          <p dir="rtl">الزراعة</p>
                        </td>
                        <td>
                          <p dir="rtl">60%:65%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">طب الفم والأسنان</p>
                        </td>
                        <td>
                          <p dir="rtl">65%:70%</p>
                        </td>
                        <td>
                          <p dir="rtl">الإعلام</p>
                        </td>
                        <td>
                          <p dir="rtl">65%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الصيدلة</p>
                        </td>
                        <td>
                          <p dir="rtl">65%:70%</p>
                        </td>
                        <td>
                          <p dir="rtl">الاقتصاد والعلوم السياسية</p>
                        </td>
                        <td>
                          <p dir="rtl">70%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">العلاج الطبيعي</p>
                        </td>
                        <td>
                          <p dir="rtl">60%:65%</p>
                        </td>
                        <td>
                          <p dir="rtl">الآداب</p>
                        </td>
                        <td>
                          <p dir="rtl">50%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الطب البيطري</p>
                        </td>
                        <td>
                          <p dir="rtl">60:65%</p>
                        </td>
                        <td>
                          <p dir="rtl">الآثار</p>
                        </td>
                        <td>
                          <p dir="rtl">50%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">التمريض</p>
                        </td>
                        <td>
                          <p dir="rtl">50%</p>
                        </td>
                        <td>
                          <p dir="rtl">التجارة</p>
                        </td>
                        <td>
                          <p dir="rtl">50%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الهندسة</p>
                        </td>
                        <td>
                          <p dir="rtl">65%</p>
                        </td>
                        <td>
                          <p dir="rtl">الحقوق</p>
                        </td>
                        <td>
                          <p dir="rtl">50%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الحاسبات والذكاء الاصطناعي</p>
                        </td>
                        <td>
                          <p dir="rtl">60%:65%</p>
                        </td>
                        <td>
                          <p dir="rtl">التربية النوعية</p>
                        </td>
                        <td>
                          <p dir="rtl">50%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">الفنون التطبيقية</p>
                        </td>
                        <td>
                          <p dir="rtl">50%:55%</p>
                        </td>
                        <td>
                          <p dir="rtl">التربية</p>
                        </td>
                        <td>
                          <p dir="rtl">50%</p>
                        </td>
                      </tr>
                      <tr>
                        <td>
                          <p dir="rtl">العلوم</p>
                        </td>
                        <td>
                          <p dir="rtl">50%:55%</p>
                        </td>
                        <td>
                          <p dir="rtl">اللغات والترجمة</p>
                        </td>
                        <td>
                          <p dir="rtl">55%</p>
                        </td>
                      </tr>
                    </tbody>
                  </table>
                </div>

                <h2 dir="rtl" id="id-spnpgcv">
                  مصروفات الجامعات الخاصة لطلاب الشهادات المعادلة
                </h2>

                <p dir="rtl">
                  تشهد الجامعات الخاصة في مصر إقبالًا ملحوظًا من الطلاب الحاصلين
                  على الشهادات المعادلة، سواء من الدول العربية أو الحاصلين على
                  شهادات دولية، لما توفره من بيئة تعليمية متطورة وبرامج أكاديمية
                  حديثة تراعي المعايير العالمية.
                </p>

                <p dir="rtl">
                  تُعد تكاليف الدراسة في الجامعات الخاصة في مصر لهذه الفئة من
                  الطلاب رمزية إذا ما قورنت بالإمكانات التي توفرها الجامعات من
                  معامل وتجهيزات حديثة وهيئة تدريس متميزة، إلى جانب اعتماد
                  الشهادات وتنوع التخصصات المتاحة.
                </p>

                <p dir="rtl">
                  تختلف المصروفات من جامعة لأخرى وفقًا لطبيعة البرنامج الأكاديمي
                  والتخصص، وفيما يلي توضيح لأبرز الرسوم الدراسية التي أقرتها بعض
                  الجامعات الخاصة لطلاب الشهادات المعادلة (للوافدين):
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    الجامعة الأمريكية بالقاهرة: تصل رسوم الساعة المعتمدة إلى 735
                    دولار أمريكي.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    الجامعة البريطانية في مصر: تكون تكاليف الدراسة في الجامعات
                    الخاصة في مصر بالجامعة البريطانية ما بين 2269 إلى 11970 جنيه
                    استرليني لكل عام دراسي.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جامعة مصر الدولية: تتفاوت تكاليف الدراسة ما بين 4000 إلى
                    13200 دولار أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جامعة السلام: تتراوح رسوم الدراسة من 2385 إلى 3578 دولار
                    أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جامعة 6 أكتوبر: تتدرج تكاليف الدراسة من 2240 إلى 7500 دولار
                    أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جامعة نيو جيزة: تبلغ مصاريف الدراسة ما بين 10000 إلى 19600
                    دولار أمريكي لكل عام دراسي.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جامعة أكتوبر للعلوم الحديثة والآداب: تتراوح تكاليف الدراسة
                    ما بين 6500 إلى 23800 دولار أمريكي سنويًا.
                  </li>
                </ul>

                <p dir="rtl">
                  في نهاية المطاف، يمكن القول إن تكاليف الدراسة في الجامعات
                  الخاصة في مصر تعتبر رمزية خاصة عند النظر إلى ما تقدمه هذه
                  الجامعات من جودة تعليم، وتجهيزات حديثة، وبيئة أكاديمية داعمة
                  تواكب المعايير العالمية، فالكثير من البرامج والتخصصات مصممة
                  بعناية لتأهيل الطلاب علميًا وعمليًا، مما يجعل ما يُنفق على
                  الدراسة استثمارًا حقيقيًا في مستقبل أكاديمي ومهني واعد، لذلك
                  سجل في الجامعات المصرية الخاصة بك سهولة مع شركة ايجيك الأولى
                  عالميًا في تقديم الخدمات التعليمية، للتواصل اضغط على ايقونة
                  الواتساب.
                </p>

                <h2 dir="rtl" id="id-z19w8ky">
                  الأسئلة الشائعة (FAQ) حول تكاليف الدراسة في الجامعات الخاصة في
                  مصر
                </h2>

                <h3 dir="rtl">
                  ما هي متوسط أسعار دخول الجامعات الخاصة في مصر؟
                </h3>

                <div></div>
                <p dir="rtl">
                  تختلف تكاليف الدراسة في الجامعات الخاصة في مصر من جامعة إلى
                  أخرى، وتتفاوت أيضًا هذه الرسوم وفقًا لنوع التخصص، فيما يلي
                  نعرض الجامعات الخاصة المعتمدة في مصر واسعارها:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    جامعة 6 أكتوبر: من 2240 إلى 7500 دولار أمريكي لكل عام دراسي.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    الجامعة الأمريكية بالقاهرة: 735 دولار أمريكي لكل ساعة
                    معتمدة.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    الجامعة البريطانية في مصر: من 2269 إلى 11970 جنيه إسترليني
                    سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جامعة أكتوبر للعلوم الحديثة والتكنولوجيا: من 6500 إلى 23800
                    دولار أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جامعة مصر الدولية: من 4000 إلى 13200 دولار أمريكي سنويًا.
                  </li>
                  <li dir="rtl" style="text-align: right">
                    جامعة نيو جيزة: 10000 إلى 19600 دولار أمريكي سنويًا.
                  </li>
                </ul>

                <h3 dir="rtl">
                  هل تختلف أسعار الجامعات الخاصة بين المصريين والوافدين؟
                </h3>

                <p dir="rtl">
                  نعم، تختلف تكاليف الدراسة في الجامعات الخاصة في مصر بين الطلاب
                  المصريين والوافدين، حيث تعتمد كل جامعة على نظام مالي خاص بها
                  يُحدد المصروفات الدراسية وفقًا لجنسية الطالب ونوع الشهادة
                  الحاصل عليها.
                </p>

                <p dir="rtl">
                  وتُعلن الجامعات هذه المصروفات بشكل رسمي ضمن لوائح القبول
                  السنوية، مع توضيح القيم المقررة لكل فئة، سواء للطلاب المصريين،
                  أو الحاصلين على الشهادات المعادلة، أو الوافدين من الخارج،
                  ويهدف هذا التنوع إلى مراعاة الاختلافات في الخلفيات التعليمية
                  وتنظيم إجراءات القبول، دون أن يؤثر ذلك على جودة الخدمة
                  التعليمية المقدمة لجميع الطلاب.
                </p>

                <h3 dir="rtl">هل المصروفات تشمل الإقامة أو الانتقالات؟</h3>

                <p dir="rtl">
                  لا، تكاليف الدراسة في الجامعات الخاصة في مصر في الجامعات
                  الخاصة في مصر لا تشمل عادة الإقامة أو الانتقالات، فالمبالغ
                  المُعلنة على المواقع الرسمية أو في لوائح الجامعات تقتصر غالبًا
                  على الرسوم الأكاديمية فقط، والتي تغطي الدراسة والمقررات
                  والمعامل والتسجيل وبعض الخدمات الإدارية، أما الإقامة في المدن
                  الجامعية (إن وُجدت)، أو السكن الخارجي، وكذلك الانتقالات
                  اليومية أو المواصلات من وإلى الجامعة، فهي خدمات إضافية برسوم
                  مستقلة، وتختلف تكلفتها من جامعة لأخرى.
                </p>

                <h3 dir="rtl">هل هناك منح أو خصومات في الجامعات الخاصة؟</h3>

                <p dir="rtl">
                  نعم، توفر الجامعات الخاصة في مصر فرصًا متعددة للمنح والخصومات،
                  في إطار سعيها لتخفيف الأعباء المالية عن الطلاب وأسرهم، ودعم
                  استمرارية التعليم الجامعي دون أن تكون التكلفة عائقًا، وتُمنح
                  هذه التسهيلات وفق معايير محددة تضعها كل جامعة، وتُعد جزءً من
                  التزام المؤسسات التعليمية بتشجيع الطلاب على التفوق والاستقرار
                  الدراسي، مع مراعاة الظروف المختلفة لكل حالة.
                </p>

                <h3 dir="rtl">ماهي أرخص الجامعات الخاصة المعتمدة في مصر؟</h3>

                <p dir="rtl">
                  تضم مصر العديد من الجامعات الخاصة المعتمدة والمعترف بها محليًا
                  ودوليًا، وتتسم تكاليف الدراسة في الجامعات الخاصة في مصر بأنها
                  معتدلة وتتوافق مع ميزانية الطلاب، إليك أبرز هذه الجامعات:
                </p>

                <ul>
                  <li dir="rtl" style="text-align: right">
                    جامعة بدر بالقاهرة
                  </li>
                  <li dir="rtl" style="text-align: right">جامعة المستقبل</li>
                  <li dir="rtl" style="text-align: right">
                    جامعة الدلتا للعلوم والتكنولوجيا
                  </li>
                  <li dir="rtl" style="text-align: right">
                    الجامعة الأمريكية بالقاهرة
                  </li>
                  <li dir="rtl" style="text-align: right">
                    الجامعة البريطانية في مصر
                  </li>
                  <li dir="rtl" style="text-align: right">جامعة نيو جيزة.</li>
                  <li dir="rtl" style="text-align: right">جامعة 6 أكتوبر</li>
                  <li dir="rtl" style="text-align: right">جامعة السلام</li>
                  <li dir="rtl" style="text-align: right">
                    جامعة أكتوبر للعلوم الحديثة والآداب
                  </li>
                </ul>
              </div>
            </div>
            <!-- Form -->
            <div class="relative col-span-12 xl:col-span-5">
              <div class="sticky top-8" id="tableOfContent">
                <div
                  class="top-24 tableOfContents hidden xl:block overflow-y-auto mb-5 bg-white/60 backdrop-blur-md border-s-4 border-main-green rounded-2xl shadow-md p-5 transition-all duration-300 hover:shadow-lg"
                >
                <h3 class="text-primary-text-color font-bold mb-4 text-lg tracking-wide border-b pb-2 border-gray-200">{{ __('blog.table_of_contents') }}</h3>
                <div class="overflow-y-auto max-h-[300px] custom-scrollbar ps-3">
                  <ul class="space-y-2 text-sm text-alternate-text-color" dir="{{ app()->getLocale() == 'ar' ? 'rtl' : 'ltr' }}"></ul></div>
                </div>
                <form
                  data-validate="true"
                  class="border-2 border-primary-text-color/20 rounded-[50px] p-8 lg:p-10 bg-white shadow-sm"
                >
                  <!-- Form Header -->
                  <div class="text-center mb-8">
                    <h3
                      class="text-primary-text-color text-[22px] lg:text-[26px] font-bold mb-4 leading-relaxed"
                    >
                      {{ __('blog.form_title') }}
                    </h3>
                    <p
                      class="text-alternate-text-color text-[14px] lg:text-[16px] leading-relaxed"
                    >
                      {{ __('blog.form_desc') }}
                    </p>
                  </div>

                  <!-- Name Field -->
                  <div class="mb-10 relative">
                    <input
                      type="text"
                      id="name"
                      name="name"
                      placeholder="{{ __('blog.placeholder_name') }}"
                      class="main-input preventNumber"
                    />
                  </div>

                  <!-- Email Field -->
                  <div class="mb-10 relative">
                    <input
                      type="email"
                      id="email"
                      name="email"
                      placeholder="{{ __('blog.placeholder_email') }}"
                      class="main-input"
                    />
                  </div>

                  <!-- Phone Field -->
                  <div class="mb-10 relative phone-input">
                    <input
                      type="tel"
                      id="phone"
                      name="phone"
                      placeholder="{{ __('blog.placeholder_phone') }}"
                      class="main-input"
                    />
                    <input
                      type="tel"
                      class="hidden"
                      readonly
                      id="actualPhone"
                    />
                  </div>

                  <!-- Message Field -->
                  <div class="mb-10 relative">
                    <textarea
                      id="message"
                      name="message"
                      rows="6"
                      placeholder="{{ __('blog.placeholder_message') }}"
                      class="main-input resize-none"
                    ></textarea>
                  </div>

                  <!-- Submit Button -->
                  <div class="text-center">
                    <button
                      type="submit"
                      class="px-12 py-3 bg-[#EF4444] hover:bg-[#DC2626] text-white font-bold text-[18px] rounded-[25px] transition-all duration-300 transform hover:scale-105 hover:shadow-lg cursor-pointer"
                    >
                      {{ __('blog.submit_button') }}
                    </button>
                  </div>
                </form>
              </div>
              <div
                class="img-bg absolute bottom-0 start-0 translate-y-full rtl:-scale-x-[1]"
                data-hide-mobile
              >
                <svg
                  role="presentation"
                  xmlns="http://www.w3.org/2000/svg"
                  xmlns:xlink="http://www.w3.org/1999/xlink"
                  width="505"
                  height="170"
                  viewBox="0 0 505 170"
                >
                  <image
                    width="505"
                    height="170"
                    xlink:href="data:image/png;base64,"
                  ></image>
                </svg>
              </div>
            </div>
          </div>
          <!-- Related Blogs -->
          <div class="related pt-[100px]">
            <h6
              class="main-title-ui mx-auto mb-20 before:left-0 before:w-[20%] relative after:absolute after:left-0 after:-translate-x-[65%] after:translate-y-[-20%] after:top-0 after:w-10 after:h-10 after:bg-[url('../../assets/images/related-blogs/realated-img.svg')] after:hidden lg:after:block after:bg-no-repeat after:bg-contain after:bg-center"
            >
              {{ __('blog.related_articles') }}
            </h6>

            <div
              class="grid grid-cols-1 gap-x-8 gap-y-5 md:grid-cols-2 w-full lg:w-[75%] mx-auto"
            >
              <article class="blog shadow-md rounded-[30px] overflow-hidden">
              <a href="#">
              <figure class="overflow-hidden w-full h-[150px] relative">
                <img
                  class="h-full w-full object-cover"
                  src="{{asset('assets/images/related-blogs/2.webp')}}"
                  alt="Blog Featured Image"
                  loading="lazy" />
              </figure>
              <div class="content bg-[#f3fffb] py-4 px-3">
                <p
                  class="line-clamp-2 text-description text-alternate-text-color mb-2 border-b border-gray-200 pb-2">
                  الفرق بين دكتوره الفلسفه و دكتوره العلوم
                </p>
                <div class="flex items-center gap-x-2">
                  <img
                    class="w-5 h-5"
                    src="{{asset('assets/images/related-blogs/plus.svg')}}"
                    alt="" />
                  <span class="block text-primary-text-color font-light">
                    {{ __('blog.medical_sector') }}</
                  >
                </div>
              </div>
              </a>
            </article>
              <article class="blog shadow-md rounded-[30px] overflow-hidden">
              <a href="#">
              <figure class="overflow-hidden w-full h-[150px] relative">
                <img
                  class="h-full w-full object-cover"
                  src="{{asset('assets/images/related-blogs/2.webp')}}"
                  alt="Blog Featured Image"
                  loading="lazy" />
              </figure>
              <div class="content bg-[#f3fffb] py-4 px-3">
                <p
                  class="line-clamp-2 text-description text-alternate-text-color mb-2 border-b border-gray-200 pb-2">
                  الفرق بين دكتوره الفلسفه و دكتوره العلوم
                </p>
                <div class="flex items-center gap-x-2">
                  <img
                    class="w-5 h-5"
                    src="{{asset('assets/images/related-blogs/plus.svg')}}"
                    alt="" />
                  <span class="block text-primary-text-color font-light">
                    القطاع الطبي</
                  >
                </div>
              </div>
              </a>
            </article>
            </div>
          </div>
        </div>
      </section>
@endsection

@push('scripts')
<script
      defer
      src="https://cdnjs.cloudflare.com/ajax/libs/intl-tel-input/17.0.13/js/intlTelInput.min.js"
    ></script>
    <script>
      const whatsappUrl = 'https://wa.me/YOUR_PHONE_NUMBER';
    </script>
    <script defer src="{{asset('assets/js/form-validation.js')}}" defer></script>
    <script defer src="{{asset('assets/js/blog-details.js')}}" defer></script>
@endpush